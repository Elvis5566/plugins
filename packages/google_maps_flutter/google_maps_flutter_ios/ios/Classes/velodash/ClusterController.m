...
- (void)addOrUpdateMarkers:(NSArray *)markers {
    for (NSDictionary* marker in markers) {
        ...
        if ([self checkIfMarkerExists:markerId]) {
...
- (BClusterItem*)createClusterItem: (NSDictionary*)data {
    ...
    if (anchor) {
        item.anchor = [FLTGoogleMapJSONConversions pointFromArray:anchor];
    }
    
    NSNumber* draggable = data[@"draggable"];
    if (draggable != nil) {
        item.draggable = [draggable boolValue];
    }
    
    NSNumber* flat = data[@"flat"];
    if (flat != nil) {
        item.flat = [flat boolValue];
    }
    
    NSNumber* consumeTapEvents = data[@"consumeTapEvents"];
    if (consumeTapEvents != nil) {
        item.consume = [consumeTapEvents boolValue];
    }
    
    NSNumber* rotation = data[@"rotation"];
    if (rotation != nil) {
        item.rotation = [rotation doubleValue];
    }
    
    NSNumber* visible = data[@"visible"];
    if (visible != nil) {
        item.visible = [visible boolValue];
    }
    
    NSNumber* zIndex = data[@"zIndex"];
    if (zIndex != nil) {
        item.zIndex = [zIndex intValue];
    }
